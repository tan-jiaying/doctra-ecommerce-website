...
if (isset($_REQUEST['email'])){
  ...
  $sql = "SELECT * FROM `customers` WHERE email='$email'"; // retrieve user details from database
	$helo = mysqli_query($handler1, $sql) or die(mysql_error());
	$rows = mysqli_num_rows($helo);
        if($rows!=0){ // check is email already exists in database
...
<!-- Main Content -->
  <div class="login-background">
    ...
    <!-- Registration Form -->
    <form class="signup-form" style="margin-top: 55px;" id="signup-form" action="register2.php" method="post" name="register" >
      ...
      Have an account?&nbsp;
      <a href="login2.php">Log In Now</a>
...
<script src="js/form2InputsCheck.js"></script>
<script>
